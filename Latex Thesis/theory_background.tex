...
This chapter connects economic theory with business strategy and consumer behavior.

\section{Economic Foundations of International Price Setting}
\label{sec:theory_pricing}

Geo-arbitrage happens because prices differ. To understand why consumers do it, we first need to see why firms set different prices.
...
\textcite{oberholzer2007effect} showed that file-sharing forced the music industry to innovate (leading to iTunes and Spotify). Similarly, VPN usage is a market signal. It shows a mismatch between rigid regional offers and the global internet.
\begin{itemize}
    \item \textit{Access vs. Price:} Piracy was often about access ("not available here"). Geo-arbitrage is often about price or licensing restrictions.
\end{itemize}
...
Digital platforms must balance growth (openness) with profit (control). VPN providers exploit this balance. They use the platform's content (Netflix) but bypass its payment rules. This creates a technical cycle of countermeasures:
\begin{itemize}
    \item \textbf{Coercive Strategies:} Legal threats, IP bans, and strict payment checks.
    \item \textbf{Adaptive Strategies:} Standardizing prices to remove the incentive to cheat.
\end{itemize}
