...
\usepackage{placeins}  % Float section management
\usepackage{float}     % Provides [H] specifier for exact float positioning

% Relax float placement constraints so figures/tables stay near their references
\renewcommand{\topfraction}{0.9}        % Allow up to 90% of a page to be floats (top)
\renewcommand{\bottomfraction}{0.8}     % Allow up to 80% of a page to be floats (bottom)
\renewcommand{\textfraction}{0.1}       % Require at least 10% text on a float page
\renewcommand{\floatpagefraction}{0.7}  % Float-only pages must be at least 70% full
\setcounter{topnumber}{3}               % Allow up to 3 floats at top of page
\setcounter{bottomnumber}{2}            % Allow up to 2 floats at bottom of page
\setcounter{totalnumber}{5}             % Allow up to 5 floats total per page
...
%  Strategic Archetypes (defined inline in the Discussion chapter, not as glossary entries)
...
\newglossaryentry{digitalgeoarbitrage}{
	...
	description={The practice of exploiting geographic price differences for digitally delivered subscription services (e.g., streaming, cloud storage, SaaS) by using \glspl{vpn} or similar tools to purchase subscriptions at prices intended for a different (typically lower-income) market. The intangible nature of the product eliminates shipping costs and physical border controls, making price discrimination both easier to implement and easier to circumvent. Used synonymously with ``geo-arbitrage,'' ``price hopping,'' and ``subscription hopping'' in this thesis}
}
...
\institute{Entrepreneurship and Innovation Management}
...
easily justifying the \$3--4 monthly cost of a commercial \gls{vpn} subscription on a multi-year plan.
...
``Coercive'' strategies that try to keep markets separate through \gls{technicalblocking},
...
Blocking,'' ``\gls{legalthreat},'' and ``Price Discrimination.'' Looking at data over time lets us
...
failure to meet consumer demand for convenient digital access, \gls{vpn}-enabled \gls{pricehopping}
...
2023, which coincides precisely with the broader \gls{enforcementsurge} documented in our
...
piracy, we propose that the decision to engage in geo-arbitrage can be modeled as a \gls{threelevel}.
...
capable of both defensive and \gls{adaptiveinnovation} outperform those relying on a single approach \parencite{karimi2020strategic}.
The first is \textbf{\gls{coerciveinnovation}} (Value Capture focus), which reinforces the barriers to protect the existing model through technical blocking and legal threats. The second is \textbf{adaptive innovation} (Value Proposition focus), which changes the product offer to make arbitrage irrelevant, for example through global pricing or ecosystem lock-in.
...
(3) the elasticity of user circumvention behavior to \gls{enforcementintensity}
...
The country selection followed a purposive layered sampling strategy designed to maximize
...
Data was collected using a \textbf{\gls{digitalaudit}} approach, adapting the methodology
...
``mystery shopping'' in digital markets. For each service and country, the ``Standard''
subscription price was recorded in local currency. For most services this is a
monthly subscription; for Microsoft 365 (annual plan), NordVPN (2-year + 3-month plan),
and ExpressVPN (2-year + 4-month plan), the total plan price was divided by the
plan duration (12, 27, and 28~months, respectively) to obtain the equivalent monthly cost.
...
\textbf{\gls{gioiamethodology}} \parencite{gioia2013seeking}, which organizes
...
No existing coding scheme in the literature addresses the specific intersection of
geo-blocking enforcement, geographic price discrimination, and circumvention
technology in corporate documents. The following categories were therefore
developed by the author, grounded in \textcite{sundararajan2004managing}'s
theoretical distinction between technological protection and pricing responses
to digital circumvention, and operationalized for the context of \gls{tos} analysis.
The resulting coding scheme was enforced via the
\gls{llm} system prompt, mapping the abstract concept of ``Strategic Response''
...
new \gls{tos} documents every year. To address this, a \gls{forwardfill}
...
Additionally, the model's self-reported confidence scores served as a filtering
mechanism: the final dataset achieved an average confidence score of \textbf{0.947},
with \textbf{80.5\%} of classifications exceeding a confidence threshold of 0.9.
...
balanced across service providers and document years to ensure representativeness.
...
each service's \gls{dspi} across countries was used as the ``\gls{pricediscriminationscore}'':
...
coefficients \parencite{pearson1895correlation} were calculated between the Price Discrimination Score
...
comparisons, we developed the ``\gls{fortressindex},'' the percentage of
...
The qualitative corpus comprises 25,570 sentences across 10 companies.
Figure~\ref{fig:app_company_dist} shows the distribution of clause counts by company,
while Figure~\ref{fig:app_doctype_dist} presents the breakdown by document type.
Table~\ref{tab:app_service_stats} provides the full category-level breakdown per service.
...
commercial \gls{vpn} subscription (\$3--4/month on multi-year plans), explaining the persistence
...
\textbf{Pakistan} appears cheapest ($\text{DSPI}=0.45$), it is the most
expensive for locals ($4.03\%$ of median wage across available services).
In contrast, \textbf{Turkey} ($\text{DSPI}=0.65$, \gls{ptw} $= 1.32\%$)
and \textbf{Argentina} ($\text{DSPI}=0.76$, \gls{ptw} $= 2.56\%$) show
high nominal discounts, but their \gls{ptw} values reveal that these
services still consume a notably larger share of local income than in
high-income countries (where average \gls{ptw} stays below $0.35\%$).
Argentina's particularly high ratio suggested pricing there was
...
\begin{table}[H]
	...
	\begin{tabular}{l c c c}
		\toprule
		\textbf{Country} & \textbf{Avg.\ \gls{dspi}} & \textbf{N Services} & \textbf{Avg.\ \gls{ptw} (\%)} \\
		\midrule
		Switzerland      & 1.24                      & 11                  & 0.26                          \\
		United Kingdom   & 1.04                      & 11                  & 0.31                          \\
		Germany          & 1.01                      & 11                  & 0.33                          \\
		United States    & 1.00                      & 11                  & 0.25                          \\
		Poland           & 0.77                      & 11                  & 0.86                          \\
		Argentina        & 0.76                      & 11                  & 2.56                          \\
		Turkey           & 0.65                      & 11                  & 1.32                          \\
		Ukraine          & 0.62                      & 10                  & 1.77                          \\
		Brazil           & 0.59                      & 11                  & 1.54                          \\
		Philippines      & 0.54                      & 11                  & 2.76                          \\
		Pakistan         & 0.45                      & 9                   & 4.03                          \\
		\bottomrule
	\end{tabular}
	\caption{Complete \gls{dspi} Summary by Country. \gls{ptw} = average subscription cost across all available services as \% of median monthly wage. Lower \gls{dspi} indicates cheaper markets relative to the US baseline.}
	\label{tab:app_dspi}
\end{table}

Table~\ref{tab:ptw_matrix} breaks these averages down to the individual service level,
showing the cost of each digital service as a percentage of the median monthly wage
in each country. Values above 10\% indicate that
a single subscription would consume more than a tenth of the median monthly
income; notably, only Adobe Creative Cloud exceeds this threshold,
reaching 12.93\% in Argentina, 18.49\% in the Philippines, and 27.45\% in Pakistan.

\begin{table}[H]
	\centering
	\caption{Complete \gls{ptw} Matrix: Subscription Cost as \% of Median Monthly Wage
		by Service and Country (December 2025). Cells marked ``--'' indicate
		the service is unavailable in that country. Values $\geq 10\%$ are
		highlighted in \textbf{bold}.}
	\label{tab:ptw_matrix}
	\scriptsize
	\renewcommand{\arraystretch}{1.1}
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{l *{11}{r}}
			...
			\midrule
			Switzerland      & 0.33                             & 0.26                             & 0.24                             & 0.14                            & 0.23                             & 0.20                              & 0.12                            & 1.11                              & 0.14                             & 0.05                             & 0.05                                \\
			United Kingdom   & 0.24                             & 0.29                             & 0.24                             & 0.20                            & 0.27                             & 0.24                              & 0.16                            & 1.48                              & 0.16                             & 0.06                             & 0.07                                \\
			Germany          & 0.29                             & 0.27                             & 0.23                             & 0.19                            & 0.27                             & 0.23                              & 0.17                            & 1.62                              & 0.19                             & 0.07                             & 0.07                                \\
			United States    & 0.27                             & 0.21                             & 0.20                             & 0.23                            & 0.18                             & 0.17                              & 0.13                            & 1.06                              & 0.15                             & 0.05                             & 0.06                                \\
			Poland           & 0.73                             & 0.58                             & 0.52                             & 0.16                            & 0.36                             & 0.33                              & 0.53                            & 5.25                              & 0.55                             & 0.18                             & 0.26                                \\
			Argentina        & 3.28                             & 1.88                             & 2.69                             & 1.74                            & 0.72                             & 1.31                              & 0.68                            & \textbf{12.93}                    & 1.97                             & 0.29                             & 0.64                                \\
			Turkey           & 1.22                             & 0.32                             & 1.89                             & 0.29                            & 0.42                             & 0.25                              & 1.16                            & 6.84                              & 1.13                             & 0.47                             & 0.55                                \\
			Ukraine          & 1.60                             & 0.50                             & --                               & 1.49                            & 0.98                             & 0.98                              & 1.15                            & 8.00                              & 1.48                             & 0.70                             & 0.82                                \\
			Brazil           & 1.50                             & 0.83                             & 1.56                             & 0.66                            & 0.80                             & 0.73                              & 1.41                            & 7.13                              & 1.46                             & 0.26                             & 0.58                                \\
			Philippines      & 2.18                             & 0.92                             & 1.21                             & 0.72                            & 0.82                             & 0.68                              & 1.98                            & \textbf{18.49}                    & 1.56                             & 0.86                             & 0.94                                \\
			Pakistan         & 1.13                             & 0.67                             & --                               & 0.85                            & 0.49                             & --                                & 2.71                            & \textbf{27.45}                    & 0.71                             & 1.13                             & 1.10                                \\
			\bottomrule
		\end{tabular}}
\end{table}
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{l r *{11}{r}}
			...
			\midrule
			Switzerland      & 7{,}800                            & 25.88                            & 20.23                            & 19.10                            & 11.29                           & 18.02                            & 15.71                             & 9.41                            & 86.73                             & 11.29                            & 3.89                             & 3.77                                \\
			United Kingdom   & 5{,}715                            & 13.96                            & 16.50                            & 13.96                            & 11.42                           & 15.23                            & 13.96                             & 8.99                            & 84.44                             & 8.88                             & 3.46                             & 3.79                                \\
			Germany          & 5{,}232                            & 15.25                            & 14.16                            & 11.98                            & 9.80                            & 14.16                            & 11.98                             & 8.99                            & 85.01                             & 9.80                             & 3.88                             & 3.88                                \\
			United States    & 6{,}600                            & 17.99                            & 13.99                            & 12.99                            & 14.99                           & 11.99                            & 10.99                             & 8.33                            & 69.99                             & 9.99                             & 3.56                             & 3.80                                \\
			Poland           & 1{,}675                            & 12.25                            & 9.75                             & 8.75                             & 2.75                            & 6.00                             & 5.50                              & 8.96                            & 87.86                             & 9.25                             & 3.04                             & 4.29                                \\
			Argentina        & 548                                & 18.00                            & 10.32                            & 14.76                            & 9.54                            & 3.96                             & 7.17                              & 3.72                            & 70.84                             & 10.80                            & 1.58                             & 3.49                                \\
			Turkey           & 761                                & 9.28                             & 2.43                             & 14.40                            & 2.24                            & 3.17                             & 1.92                              & 8.80                            & 52.07                             & 8.61                             & 3.59                             & 4.19                                \\
			Ukraine          & 510                                & 8.16                             & 2.57                             & --                               & 7.62                            & 4.99                             & 4.99                              & 5.85                            & 40.79                             & 7.54                             & 3.59                             & 4.19                                \\
			Brazil           & 600                                & 8.98                             & 4.98                             & 9.38                             & 3.98                            & 4.78                             & 4.38                              & 8.48                            & 42.80                             & 8.78                             & 1.58                             & 3.49                                \\
			Philippines      & 370                                & 8.08                             & 3.40                             & 4.48                             & 2.68                            & 3.04                             & 2.50                              & 7.35                            & 68.40                             & 5.76                             & 3.20                             & 3.49                                \\
			Pakistan         & 255                                & 2.88                             & 1.72                             & --                               & 2.16                            & 1.26                             & --                                & 6.90                            & 69.99                             & 1.80                             & 2.88                             & 2.79                                \\
			\bottomrule
		\end{tabular}}
\end{table}
...
\textbf{Fortress Index}: the percentage of enforcement-related clauses
(Technical Blocking + Legal Threat) relative to the total number of
\textit{strategic} (non-General Terms) sentences in a firm's documentation.
By excluding boilerplate General Terms from the denominator, the index
isolates enforcement \textit{focus} from document length. A high Fortress
Index indicates a firm that builds a ``digital fortress'' around its
geographic pricing through coercive policy language, while a low score
signals a \textit{globalist} approach that relies on pricing adaptation,
content licensing, or ecosystem lock-in instead.
...
The contrast is striking. YouTube Premium (33.68\%) devotes roughly one
in three substantive policy sentences to blocking or threatening users,
while Spotify (0.43\%) and Netflix (2.03\%) dedicate almost none.
These globalist firms address arbitrage through business model adaptation
rather than technological coercion: Spotify through increasingly harmonized
global pricing, Netflix through globally licensed original content that
reduces the need for geographic restrictions altogether.
...
(ExpressVPN, NordVPN), the same categories capture \textit{adversarial}
...
threats that justify circumvention. Both groups are kept in one ranking
...
\subsection{Evolution of Enforcement Over Time}
...
($N=525$ \gls{contentlicensing} clauses), reflecting the industry's
...
Importantly, the full \gls{ptw} matrix (Table~\ref{tab:ptw_matrix}) reveals that
the affordability gap is driven almost entirely by \textbf{Adobe Creative Cloud},
which is the only service whose monthly cost regularly exceeds 10\% of local wages
in developing economies. In Pakistan, Adobe Creative Cloud consumes 27.45\%
of the median monthly wage, while in the Philippines it reaches 18.49\% and
in Argentina 12.93\%. By contrast, once multi-year \gls{vpn} plans and annual
Microsoft 365 subscriptions are expressed on a monthly basis,
these services become comparable to consumer streaming subscriptions:
in Pakistan, NordVPN costs 1.13\% and ExpressVPN 1.10\% of the median wage,
and Microsoft 365 costs 2.71\%, all below the levels of streaming services
like Netflix (1.13\%) and Amazon Prime (0.85\%).
A similar pattern holds across all low-income markets: in the Philippines,
NordVPN (0.86\%) and ExpressVPN (0.94\%) are among the most affordable
subscriptions, and Microsoft 365 (1.98\%) is comparable to Netflix (2.18\%).
This finding suggests that, with the notable exception of Adobe Creative Cloud,
most digital subscription services, including \gls{vpn} providers and
Microsoft 365, have achieved roughly comparable monthly price points
across categories. The implication for geo-arbitrage is that
Adobe Creative Cloud's near-uniform global pricing creates the most
severe affordability barrier, making it the service where
professional users in low-income countries face the strongest motivation
to seek workarounds in order to participate in the global digital economy.
...
%  Generated Video Group (proportional evolution) 
\begin{figure}[H]
	\centering
...
\begin{enumerate}
	\item \textbf{Business Model Adaptation \& Pricing (\acrshort{bma}):}
	...
	\item \textbf{Coercive Restriction \& Legal Threat (\acrshort{crl}):}
	...
	\item \textbf{General Corporate Operations (\acrshort{gco}):}
...
\end{enumerate}
\glsunset{bma}\glsunset{crl}\glsunset{gco}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\legend{\acrshort{bma} (Adaptation), \acrshort{crl} (Enforcement), \acrshort{gco} (Operations)}
		\end{axis}
	\end{tikzpicture}
...
making the walls less necessary by producing content it fully controls \parencite{teece2010business, karimi2020strategic}.
...
approach that innovates on the \textbf{Value Proposition} \parencite{cusumano2019platform}. Unlike the Content Fortress,
...
\textbf{Amazon Prime} adopts a similar but legally distinct strategy. As shown in the
proportional evolution analysis (Figure \ref{fig:evol_video_main},
Section~\ref{sec:service_deep_dive}), Amazon relies heavily on \textbf{Regulatory Compliance} language \parencite{eu2018geoblocking, trimble2024geoblocking}.
...
\textbf{Microsoft} \parencite{teece2010business, sundararajan2004managing}. Despite having the lowest global price variance among digital service
...
network-level control, and ecosystem-integrated services leverage identity dependencies.
...
reveals a clear ``\gls{adversarialcycle},'' visualized in Figure \ref{fig:timeline}.
...
Servers'', specialized modes that disguise \gls{vpn} traffic as ordinary HTTPS browsing, using techniques
such as XOR scrambling or the Chameleon protocol, originally developed to circumvent state-level
censorship like China's Great Firewall \parencite{ensafi2015examining}. This in turn prompted platforms
to deploy more sophisticated blocking techniques (circa 2021), suggesting an ongoing
adaptation process on both sides.

The trends in our enforcement data (Table~\ref{tab:timeline_count},
Figures~\ref{fig:timeline_all}--\ref{fig:strategic_frames_evolution}) reveal four distinct
phases, each marked by a qualitative shift in the balance of power between platforms and
circumvention tools:
...
\gls{dpi}, AI to read traffic fingerprints, and browser fingerprinting
...
Our document analysis and the technical literature point to several detection layers that platforms combine in practice. At the first layer,
...
\textbf{Ecosystem platforms} (Apple, Amazon) may find that their
multi-service bundle strategy already provides the most durable form of protection against
...
As \textcite{trimble2024geoblocking} shows, this regulation
exempts video content, precisely the category where our data shows the highest
...
rather than prohibiting geo-blocking directly, regulators could incentivize fairer
cross-border pricing through transparency requirements
...
points to three potential systemic risks.
...
under data protection frameworks such as the \gls{eu}'s \gls{gdpr}. The irony is that privacy-enhancing technologies (\glspl{vpn})
...
move on from without detailed reporting. This structural asymmetry, reinforced
...
The findings of this study point to several developments that are likely to reshape the landscape of geo-blocking and digital price discrimination in the coming years.
...
As documented in Section~\ref{sec:secret_tech_race}, the arms race between
platform enforcement and circumvention tools is escalating on both sides.
A potentially disruptive development on the circumvention side is the emergence
of decentralized \gls{vpn} networks (\glspl{dvpn}) built on blockchain
infrastructure \parencite{mysterium2020dvpn}. Unlike traditional \gls{vpn}
providers that operate identifiable data centers, \glspl{dvpn} distribute
exit nodes across millions of individual devices, making IP-based blocking
fundamentally ineffective. If this technology matures, the detection layers
described in Section~\ref{sec:secret_tech_race} may become obsolete,
forcing platforms to rely entirely on identity-based and behavioral enforcement.
...
This chapter synthesizes the key findings, outlines the study's contributions
to the literature and its limitations, reflects on the methodological approach,
...
Content streaming firms enforce strict geographic blocking due to licensing requirements,
software firms rely on identity verification and legal threats (the ``Enterprise Fortress'' archetype),
and ecosystem platforms (Apple, Amazon) leverage bundle dependencies as implicit enforcement.
Framed through \gls{bmi} theory, these responses map onto a spectrum from coercive
\gls{bmi} (technological blocking, legal threats) to adaptive \gls{bmi} (pricing
innovation, ecosystem lock-in), mirroring the music industry's historical shift
from \gls{drm} enforcement to convenient streaming as a response to piracy.
The enforcement evolution analysis further shows a clear escalation peaking in 2023
(driven by YouTube), followed by stabilization as blocking became standard infrastructure.
This trajectory suggests that, as in the piracy case, adaptive strategies may prove more
sustainable than coercive ones in the long run.

\section{Contribution to Research}
This study contributes to the digital economics literature on both a methodological and a theoretical level.
...
layered sampling strategy ensures representation across income levels and
...
tolerate some level of piracy from the lower 80\%, as they were never the primary customer segment
to begin with.

The \gls{dspi} represents a snapshot of pricing data from
...
eliminated by a price adjustment or currency devaluation. The ``\gls{arbitragewindow}'' is
...
risk. Although the model's confidence scores and a 200-sentence manual review
(Section~\ref{sec:llm_methodology}) provide reassurance, the confidence values
are self-reported by the model rather than independently validated. A full
human-annotated gold standard covering the entire dataset was not feasible
given its size (approx.\ 25,000 sentences), meaning that systematic
misclassifications in edge cases cannot be fully ruled out.
...
digital economy. Second, tracking the \gls{dspi} over multiple years would
reveal how companies adjust prices in response to currency devaluations,
inflation spikes, and regulatory changes. Third, demand-side research
...
or algorithmic pricing disclosures. Additionally, experimental methods
(e.g., systematically testing \gls{vpn} access across services and regions)
could complement document-based analysis by measuring actual enforcement rates
rather than stated policies. Moreover, as detection and circumvention technologies
grow more sophisticated on both sides (Section~\ref{sec:secret_tech_race}),
researchers should investigate whether the rising cost of this arms race
eventually makes geographic price discrimination economically unsustainable,
particularly for smaller platforms that cannot afford enterprise-grade
detection infrastructure.

Ultimately, this thesis suggests that the history of digital piracy holds a clear lesson
for the geo-arbitrage conflict: coercive enforcement alone cannot resolve a problem
rooted in global income inequality and borderless technology. Just as the music industry
eventually moved from fighting piracy to offering convenient, fairly priced alternatives,
digital service providers may find that adapting their business models is more sustainable
than investing in increasingly aggressive enforcement.
