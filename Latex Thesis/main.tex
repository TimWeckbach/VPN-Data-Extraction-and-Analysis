...
\newglossaryentry{forwardfill}{
	...
	description={A data imputation strategy applied in the longitudinal \gls{tos} analysis where a document's clauses are assumed to remain valid for subsequent years until a new version is detected, distinguishing between ``missing data'' and ``persistent rules''}
}
...
\begin{abstract}
	...
	consumer-driven geo-arbitrage using \gls{vpn}s in digital subscription markets. Using a mixed-methods design, the
	...
	Analysis of up to 11 digital services across 11 countries shows large price
	...
	while effectively cheap for international arbitrageurs, these services are often more expensive
	for local consumers relative to their income.

	In the second part, an \gls{llm} (Gemini 3 Flash)
	...
	determined by business model architecture rather than by the scale of price differences.
	...
	2022 (driven primarily by YouTube), signaling an escalating arms race between platform control and
	technical bypass. The research concludes that adaptive strategies, such as pricing innovation
	and reduced price discrimination, may prove more sustainable than coercive enforcement,
	mirroring the music industry's historical shift from \gls{drm} enforcement to convenient streaming.
\end{abstract}
...
accessible and deliver everywhere similiar value,
...
economic rationale for price discrimination in the first place, potentially losing
...
\begin{itemize}
	...
	\item \textit{Industry Response:} The music industry eventually adapted with streaming services.
	      Whether companies will adopt similar adaptive responses to geo-arbitrage is a central question of this thesis.
\end{itemize}
...
The magnitude of these differences creates a ``above-normal'' profit margin
...
which added 700 observations (2.8\% of the time-series dataset),
...
\begin{description}
	...
	\item[Enforcement Evolution:] The year-by-year analysis reveals a clear escalation in
