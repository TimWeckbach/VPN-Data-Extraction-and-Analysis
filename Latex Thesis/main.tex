...
\textbf{Pakistan} appears cheapest ($\text{DSPI}=0.45$), it is by far the most
expensive for locals ($13.88\%$ of median wage across available services). In contrast, \textbf{Turkey} ($\text{DSPI}=0.65$)
and \textbf{Argentina} ($\text{DSPI}=0.76$) show high nominal discounts,
but Argentina's high \gls{ptw} ($5.48\%$) suggested pricing there was
...
\textbf{Fortress Index}. The idea is simple: of all the substantive policy
statements a firm makes (excluding routine legal boilerplate), how many are
about blocking users or threatening them with consequences? A firm that
dedicates a third of its policy language to Technical Blocking and Legal
Threat clauses is, in effect, building a ``digital fortress'' around its
geographic pricing. A firm that dedicates almost none of its policy language
to such clauses is pursuing a more open, \textit{globalist} approach ---
relying on pricing adaptation, content licensing, or ecosystem lock-in
rather than coercive enforcement.

Formally, the Fortress Index is the percentage of enforcement-related clauses
...
isolates enforcement \textit{focus} from document length.
...
The contrast between fortress and globalist strategies is striking.
YouTube Premium, with a Fortress Index of 33.68\%, devotes roughly one
in three substantive policy sentences to blocking or threatening users
who attempt geographic circumvention --- a clear fortress strategy.
At the other end, Spotify (0.43\%) and Netflix (2.03\%) dedicate almost
none of their policy language to coercive enforcement. These firms instead
pursue a globalist approach: Spotify has moved toward increasingly
harmonized global pricing, while Netflix has invested in globally licensed
original content that reduces the need for geographic restrictions
altogether. The implication is that firms with a low Fortress Index are
not necessarily ignoring arbitrage; rather, they are addressing it through
business model adaptation rather than technological coercion.
...
the affordability gap is driven almost entirely by enterprise software and
\gls{vpn} services rather than by consumer streaming subscriptions.
In Pakistan, for instance, consumer-facing services such as Netflix (1.13\%),
YouTube (0.67\%), and Spotify (0.49\%) remain below 1.5\% of the median monthly wage,
whereas Microsoft 365 consumes 32.47\%, Adobe Creative Cloud 27.45\%, and
NordVPN and ExpressVPN each exceed 30\%. A similar pattern holds across all
low-income markets: in the Philippines, streaming subscriptions stay below
2.2\% of monthly income, while Microsoft 365 reaches 23.83\% and
ExpressVPN 26.41\%. This divergence suggests that consumer streaming platforms
have largely succeeded in calibrating prices to local purchasing power,
whereas enterprise software providers and \gls{vpn} services maintain
near-global pricing that is effectively inaccessible at the median income
level in developing economies. The implication for geo-arbitrage is that
the strongest affordability-driven motivation to circumvent geographic
pricing may originate not in entertainment streaming but in professional
tools that workers and small businesses in low-income countries need to
...
Table~\ref{tab:ptw_matrix} presents the complete \gls{ptw} matrix, showing
the cost of each digital service as a percentage of the median monthly wage
in each country. This table complements the nominal USD prices
(Table~\ref{tab:dspi_usd}) and the \gls{dspi} indices (Table~\ref{tab:dspi_matrix})
by expressing prices in affordability terms. Values above 10\% indicate that
a single subscription would consume more than a tenth of the median monthly
income, highlighting the severity of the Affordability Paradox in low-income markets.
...
\begin{table}[ht]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{l *{11}{r}}
			...
			Argentina        & 3.28                             & 1.88                             & 2.69                             & 1.74                            & 0.72                             & 1.31                              & 8.15                            & \textbf{12.93}                    & 1.97                             & 7.77                             & \textbf{17.83}                      \\
			Turkey           & 1.22                             & 0.32                             & 1.89                             & 0.29                            & 0.42                             & 0.25                              & \textbf{13.88}                  & 6.84                              & 1.13                             & \textbf{12.73}                   & \textbf{15.41}                      \\
			Ukraine          & 1.60                             & 0.50                             & --                               & 1.49                            & 0.98                             & 0.98                              & \textbf{13.76}                  & 8.00                              & 1.48                             & \textbf{19.00}                   & \textbf{22.99}                      \\
			Brazil           & 1.50                             & 0.83                             & 1.56                             & 0.66                            & 0.80                             & 0.73                              & \textbf{16.97}                  & 7.13                              & 1.46                             & 7.11                             & \textbf{16.29}                      \\
			Philippines      & 2.18                             & 0.92                             & 1.21                             & 0.72                            & 0.82                             & 0.68                              & \textbf{23.83}                  & \textbf{18.49}                    & 1.56                             & \textbf{23.35}                   & \textbf{26.41}                      \\
			Pakistan         & 1.13                             & 0.67                             & --                               & 0.85                            & 0.49                             & --                                & \textbf{32.47}                  & \textbf{27.45}                    & 0.71                             & \textbf{30.48}                   & \textbf{30.66}                      \\
			\bottomrule
		\end{tabular}}
\end{table}
