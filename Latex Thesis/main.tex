...
\begin{abstract}
	...
	study first measures the nominal economic incentive for \gls{pricehopping} through the \gls{dspi},
	a price ratio index benchmarked against the US market.
	Analysis of up to 11 digital services across 11 countries shows large nominal price
	differences, with subscriptions in markets like Turkey and Pakistan offered at discounts of
	up to 90\% compared to Western baselines. A complementary \gls{ptw} ratio then reveals an ``Affordability Paradox'':
	while nominally cheap for international arbitrageurs, these services are often more expensive
	for local consumers relative to their income. In the second part, an \gls{llm} (Gemini 3 Flash)
	processed over 25,000 sentences from company documents and \gls{tos} to identify coercive
	and adaptive \gls{bmi} responses. The findings show that enforcement strategies are primarily
	determined by business model architecture rather than by the magnitude of price differences.
	Streaming providers use technical blocking to satisfy licensing rules, while software
	firms rely on identity checks and ecosystem platforms leverage bundle dependencies as implicit
	enforcement. Results show a sharp spike in technical countermeasures beginning in
	2022 (driven primarily by YouTube), signaling an escalating arms race between platform control and technical bypass. The research
	concludes that adaptive strategies, such as pricing innovation and reduced price discrimination,
	may prove more sustainable than coercive enforcement, mirroring the music industry's
	historical shift from \gls{drm} enforcement to convenient streaming.
\end{abstract}
...
This chapter gives an overview of the research and why it matters.
...
geographical borders, companies in this space commonly use, among other price discrimination strategies, geographically segmented pricing
...
This fragmentation of the digital economy into regional pricing zones reflects broader
...
the markets where geo-arbitrage is most lucrative.
...
hopping,'' ``licensing hopping,'' or ``subscription hopping'' (used synonymously in this thesis), is a form of consumer-driven market arbitrage that takes
...
assessments as commercially sensitive, the scale of the incentive is clear
...
technology to circumvent barriers. This forces companies to rethink their revenue models and develop
new business strategies, just as the rise of piracy ultimately led to the
emergence of new streaming business models such as Spotify and Netflix.
...
mechanisms as proprietary, rendering them a
...
\begin{enumerate}
	\item \textbf{Economic Imperative:} To maximize market penetration and revenue, firms must offer
	      substantially lower prices in emerging markets while maintaining premium pricing in wealthier regions.
	\item \textbf{Technical Reality:} The internet enables users to circumvent geographic
	      restrictions using readily accessible \gls{vpn} services.
\end{enumerate}
...
Raising prices in low-income markets to reduce the arbitrage incentive undermines the
economic rationale for price discrimination in the first place, potentially ceding
...
users firms seek to retain. This creates what can be described as a
...
\section[Research Questions (RQs)]{Research Questions (\glspl{rq})}
...
\begin{description}
	\item[\gls{rq}1 (The Economic Incentive):] To what extent does international price
	      differentiation for digital services deviate from local purchasing power (based on local wages),
	      and what magnitude of economic incentive does this create for consumer-driven geo-arbitrage?
\end{description}
...
\begin{description}
	\item[\gls{rq}2 (The Strategic Response):]
	      How do digital service providers strategically frame the issue of circumvention, and to what
...
\begin{itemize}
	\item \textit{Condition 1: Market Segmentation.}
	      The firm must be able to distinguish between consumer groups based on observable
	      characteristics (such as IP address or billing location).
	\item \textit{Condition 2: No Arbitrage.}
	      The firm must be able to prevent the resale or transfer of the good between segments,
	      a condition based on the economics of intellectual property rights \parencite{stiglitz2008economic}.
\end{itemize}
...
This study also suggests that companies inadvertently segment their customers into sub-groups
...
who prefer convenience. In this way, technical friction functions as a self-selection mechanism:
companies retain high revenue from the majority of users while tolerating marginal losses from a small
cohort of technically proficient users.
...
almost entirely a function of deliberate corporate strategy rather than cost-driven economic forces.
...
It is important to distinguish between two types of price dispersion.
...
wealthy markets not because costs are higher, but because consumers' willingness to pay is greater.
...
pattern can be contextualized through the history of digital piracy.
...
This fragmentation has been described as a ``Splinternet'' \parencite{masnick2019splinternet},
...
are interlinked rather than independent phenomena.
...
uniform access, the incentive for \gls{vpn}-based circumvention would diminish,
just as affordable streaming reduced the incentive for file-sharing.
...
when enforcement tightens. However, a key difference emerges between piracy and geo-arbitrage:
\begin{itemize}
	\item \textit{Access vs. Price:} Piracy was often about getting content that wasn't
	      available at all, while geo-arbitrage concerns obtaining a more favorable price. Piracy involved
	      no payment, whereas geo-arbitrage involves payment at a reduced rate.
	\item \textit{Legal Status:} Piracy is clearly illegal, while geo-arbitrage is a gray zone.
	      Users are paying for real accounts, but they misrepresent their geographic location.
	\item \textit{Industry Response:} The music industry eventually adapted with streaming services.
	      Whether companies will adopt analogous adaptive responses to geo-arbitrage is a central question of this thesis.
\end{itemize}
...
\begin{enumerate}
	\item \textbf{Individual Level (Rational Choice / Personal Risk):}
	      The consumer performs a cost-benefit analysis. The financial gain
	      (e.g., a 70\% discount on Netflix from Turkey) is weighed against the perceived probability
	      ...
	      (illustratively, 20\% to 30\%), the worst-case outcome (account termination with refund
	...
	\item \textbf{Inter-personal Level (Social Influence):}
	      ...
	      sophisticated users. This ``knowledge diffusion'' effect means that as communities
	      grow, the effective barrier to entry falls, creating a positive feedback loop between
	      adoption and accessibility.
	\item \textbf{Societal Level (Moral Intensity):}
...
\begin{enumerate}
	\item \textbf{Value Proposition (What is offered):}
	...
	\item \textbf{Value Delivery (How it is reached):}
	      The channels and technical infrastructure used to deliver the value. This includes the streaming platform,
	      the Content Delivery Network (CDN) (where latency and infrastructure variability play
	      ...
	      delivery infrastructure itself a point of control.
	\item \textbf{Value Capture (How money is made):}
	      The revenue model and the mechanisms to sustain profitability \parencite{amit2012value}.
	      This encompasses the pricing strategy (e.g., price discrimination) and
...
if enough users circumvent regional restrictions, firms may be forced to rethink what they offer in each market
...
\begin{itemize}
	\item \textbf{Coercive Innovation (Value Capture Focus):}
	      Reinforcing the barriers to protect the existing model (e.g., technical blocking, legal threats).
	\item \textbf{Adaptive Innovation (Value Proposition Focus):}
	      Changing the product offer to make arbitrage irrelevant (e.g., global pricing or ecosystem lock-in).
\end{itemize}
...
dimensions:
\begin{itemize}
	\item \textbf{Protection (Coercive / Value Capture):}
	      Increasing the technological or legal costs of circumvention.
	      This attempts to \textit{repair} the broken Value Capture mechanism.
	\item \textbf{Pricing (Adaptive / Value Proposition):}
	      Adjusting the business model (pricing, versioning) to lower
	      the economic incentive for arbitrage. This effectively
	      \textit{innovates} the Value Proposition to be less sensitive to location.
\end{itemize}
...
and some level of ``tolerated arbitrage'' is economically rational.
...
\begin{itemize}
	\item \textbf{Coercive Strategies:} Legal threats embedded in \gls{tos},
	      IP address blocking, payment verification requirements, and strict geographic
	      checks on billing addresses. These strategies aim to restore the segmentation
	      condition required for price discrimination, but they impose costs on both the platform
	      (technology investment, user friction) and legitimate users (false positives, reduced portability).
	\item \textbf{Adaptive Strategies:} Standardizing global prices to remove the
	      arbitrage incentive, creating ecosystem lock-in through hardware integration
	      (e.g., Apple's approach), or developing content exclusive to specific regions
	      rather than restricting access to a global catalog. These strategies accept some
	      loss of price discrimination power in exchange for reduced enforcement costs and improved user experience.
\end{itemize}
...
\begin{enumerate}
	\item \textbf{Theoretical Isolation:}
	      ...
	      fragmentation as a broader challenge in digital economics, noting that the field
	...
	\item \textbf{Lack of Quantification:}
	      ...
	      technology journalism, systematic quantification of the arbitrage incentive
...
blocking intensity. This study provides a thorough analysis of geo-arbitrage that has been absent
from the literature.
...
used to investigate both the economic incentive for geo-arbitrage (RQ1) and the strategic
responses of digital service providers (RQ2). The methods were chosen based on both
...
problem (the arbitrage incentive) and then look at the \textit{strategic responses} of
...
This phase established the dependent variable (the arbitrage incentive) by collecting
...
Terms of Service, finding the strategic frameworks firms used to manage or fight this variance.
...
To construct the DSPI, a representative basket of 11 digital services was selected:
...
These cover Video on Demand, Music Streaming, Software/Gaming, and VPN services.
...
despite their price volatility, as they represent extreme cases that illuminate the boundaries
...
strategic framing provides a useful counterpoint to the defensive postures of content and software providers.
...
(3) publicly available Terms of Service documents spanning at least three years of the study period (2020--2025).
...
A virtual presence was established in each target country using a commercial VPN service to
...
\textbf{Stage 1: The DSPI (Nominal Price Index).} All local prices were
converted to a common currency (USD) using market exchange rates (recorded in December 2025).
The DSPI was then calculated as the ratio of the local price to the US baseline price.
A DSPI of 1.0 indicates price parity with the US market, while a DSPI $<$ 1.0 indicates a
cheaper market (a potential arbitrage source), and a DSPI $>$ 1.0 indicates a more expensive market.
...
\textbf{Stage 2: The PTW Ratio (Real Affordability).} As a separate complementary metric,
...
to standard PPP adjustment, arguing it better reflects subscription goods' affordability
relative to disposable income in the specific context of digital services.

Note that a DSPI of 1.0 (Nominal Parity) does not imply equal affordability.
...
(requiring a larger percentage of their income). Thus, the arbitrage incentive persists
even at nominal parity if the local price is structured to be affordable for the local
median earner. This distinction between nominal price (captured by the DSPI) and real
...
\section{Phase 2 (cont.): Automated Text Classification}
...
reasoning abilities and better context awareness. Unlike Natural Language Inference
(NLI) models, which classify based on entailment probabilities between premises
and hypotheses, generative LLMs can interpret complex sentence structures
...
has confirmed the viability of LLMs for automated legal analysis tasks \parencite{carneiro2025automated}.

Key advantages observed during the model transition were:
\begin{itemize}
	\item \textbf{Contextual Understanding}: The ability to distinguish
	      between benign references to ``account suspension'' (e.g., for fraud)
	      and strategic ``Legal Threats'' tailored to prevent cross-border usage.
	\item \textbf{Zero-Shot Performance}: The model demonstrated high accuracy
	      without extensive fine-tuning, utilizing a robust system prompt to align with
	      the theoretical categories defined in Chapter~\ref{chap:theory}.
	\item \textbf{Efficiency}: The ``Flash'' architecture provided a high
	      throughput, enabling the processing of the entire dataset (approx. 25,000 sentences) within a reasonable
	      timeframe of only a few days when done in batching.
\end{itemize}
...
LLM system prompt. This scheme maps the abstract concept of ``Strategic Response''
...
\begin{description}
	...
	\item[Frame: Security Risks] (Service Provider Frame) Arguments that VPNs/Proxies
	      are unsafe, malicious, or compromise user data.
	\item[Frame: Privacy/Security] (VPN Provider Frame) Arguments focusing on encryption,
...
\begin{description}
	\item[Action: Technical Blocking] Active technological measures
	      to detect or block the specific use of VPNs/Proxies
	...
	\item[Action: Account Action] General punitive measures against accounts
	      ...
	      Note: in the implemented classification prompt, this category was subsumed
	      under ``Legal Threat'' for cases specifically tied to circumvention tools
...
To ensure deterministic and theoretically grounded outputs,
...
A key challenge in analyzing longitudinal Terms of Service data is
the sparsity of document updates, as companies do not release
new ToS documents every year. To address this, a ``Forward-Fill''
imputation strategy was applied. If a document was released in a
given year (e.g., 2020), its clauses were assumed to remain valid
for subsequent years (2021, 2022) until a new document was detected.
This ensured the dataset accurately reflected the \textit{active}
regulatory environment in any given year, distinguishing between
``missing data'' and ``persistent rules.''

Also, while ``General Terms'' (standard legal boilerplate)
constitute approximately 94\% of the dataset, they are retained in the
master dataset to preserve the full document structure.
However, for visual clarity, these terms are frequently excluded
from strategic trend graphs to focus on specific enforcement categories.
...
between the two models, confirming the need for a modern LLM with
large context windows for this specific task, consistent with recent
findings on LLM performance in text annotation \parencite{gilardi2023chatgpt,hakimi2023legal}.

\subsubsection{Agreement Analysis}
The comparison revealed an exceedingly low agreement rate of
...
\begin{enumerate}
	\item \textbf{Gemini Performance:} The Gemini model correctly
	      identified that approximately \textbf{94\%} of the dataset
	      consisted of legal boilerplate, categorized as ``General Terms.''
	      It successfully distinguished specific enforcement clauses from general legal language.
	\item \textbf{BERT Performance:} The BERT model showed
	      ``Over-Sensitivity,'' frequently assigning specific strategic
	      tags based on the presence of individual keywords rather than semantic context.
\end{enumerate}

Specific examples of BERT's misclassification included:
\begin{itemize}
	\item \textbf{Legitimate Portability:}
	      BERT flagged 7,853 sentences as ``Legitimate Portability'' that were merely ``General Terms.''
	\item \textbf{Account Action:}
	      BERT flagged 6,134 ``General Terms'' sentences as ``Account Action.''
\end{itemize}
...
While the automated LLM pipeline provided scalable classification across
...
\begin{enumerate}
	\item \textbf{Validation:} Verifying the LLM classifications against human
...
This triangulation between automated and manual coding strengthens
...
contribution, as it linked the \textit{magnitude} of the economic
incentive (measured by the \gls{dspi}) with the \textit{nature} of the strategic
...
thus a stronger arbitrage incentive. Correlation matrices were generated
...
rhetoric of ``Fortress'' strategy firms (high blocking) against
``Globalist'' strategy firms (price harmonization), identifying
the key markers of each business model archetype.
This comparative framework, developed inductively from the data,
...
Second, we present the \gls{dspi} results to quantify the arbitrage incentive across services
and countries (Section~\ref{sec:dspi_results}). Third, we analyze the qualitative classification
results to map enforcement strategies and their temporal evolution
...
($N=25{,}570$, Table~\ref{tab:app_service_stats}) includes 700 forward-filled
observations added to ensure continuity of Terms of Service coverage across years:
when a service did not publish an updated ToS in a given year, the most recent
version was carried forward at the individual document level until a replacement
was published. The temporal analysis in
Table~\ref{tab:qual_timeline_complete} ($N=25{,}345$) draws from the same corpus
(including the 700 forward-filled observations) but excludes 225 sentences
from documents that could not be reliably assigned to a specific publication year
($25{,}570 - 225 = 25{,}345$). The combined effect of this exclusion means that
individual category counts may shift slightly between the two tables.
These differences are minor and do not affect the proportional
trends, as the excluded documents are distributed across categories consistent
with the dated subset.
...
The magnitude of these differences creates a ``super-normal'' profit margin
...
Critically, the arbitrage incentive is not static: currency fluctuations,
...
Table~\ref{tab:app_dspi} presents the complete DSPI and affordability
...
\begin{table}[ht]
	...
	\begin{tabular}{l c c c}
		...
		\bottomrule
	\end{tabular}
	\caption{Complete \gls{dspi} Summary by Country. \gls{ptw} = Netflix Standard subscription cost as \% of median monthly wage. Lower DSPI indicates cheaper markets relative to the US baseline.}
	\label{tab:app_dspi}
\end{table}

Table~\ref{tab:dspi_matrix} presents the complete per-service DSPI values across
all 11 countries in the dataset. A value of 1.00 indicates price parity with the
US baseline, values below 1.00 indicate cheaper markets (potential arbitrage sources),
and values above 1.00 indicate more expensive markets. Cells marked with ``--''
...
\begin{table}[ht]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{l *{11}{r}}
			...
			Poland           & 0.68                             & 0.70                             & 0.67                             & 0.18                            & 0.50                             & 0.50                              & 1.08                            & 1.26                              & 0.93                             & 0.82                             & 1.13                                \\
			...
			Pakistan         & 0.16                             & 0.12                             & --                               & 0.14                            & 0.10                             & --                                & 0.83                            & 1.00                              & 0.18                             & 0.81                             & 0.74                                \\
			\bottomrule
		\end{tabular}}
\end{table}
...
arbitrage incentives. In contrast, software services (Microsoft 365, Adobe Creative Cloud)
...
(YouTube Premium in Pakistan at DSPI = 0.12 is just 12\% of the US price),
while VPN services like NordVPN and ExpressVPN showed much smaller regional
...
\begin{table}[ht]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{l r *{11}{r}}
			...
			\midrule
			Switzerland      & 7{,}800                            & 25.88                            & 20.23                            & 19.10                            & 11.29                           & 18.02                            & 15.71                             & 112.94                          & 43.31                             & 11.29                            & 104.93                           & 119.37                              \\
			United Kingdom   & 5{,}715                            & 13.96                            & 16.50                            & 13.96                            & 11.42                           & 16.50                            & 13.96                             & 107.94                          & 31.72                             & 8.88                             & 93.45                            & 106.25                              \\
			Germany          & 5{,}232                            & 15.25                            & 14.16                            & 11.98                            & 9.80                            & 14.16                            & 11.98                             & 107.91                          & 47.14                             & 9.80                             & 104.72                           & 108.60                              \\
			United States    & 6{,}600                            & 17.99                            & 13.99                            & 12.99                            & 14.99                           & 11.99                            & 10.99                             & 99.99                           & 69.99                             & 9.99                             & 96.07                            & 106.40                              \\
			Poland           & 1{,}675                            & 12.25                            & 7.50                             & 8.75                             & 2.75                            & 6.00                             & 5.50                              & 107.50                          & 87.86                             & 9.25                             & 82.11                            & 120.13                              \\
			Argentina        & 548                                & 18.00                            & 5.02                             & 14.76                            & 9.54                            & 3.96                             & 7.17                              & 44.64                           & 70.84                             & 10.80                            & 42.60                            & 97.72                               \\
			Turkey           & 761                                & 9.28                             & 2.43                             & 14.40                            & 2.24                            & 3.17                             & 1.92                              & 105.60                          & 52.08                             & 8.61                             & 96.88                            & 117.26                              \\
			...
			Pakistan         & 255                                & 2.88                             & 1.72                             & --                               & 2.16                            & 1.26                             & --                                & 82.80                           & 68.04                             & 1.80                             & 77.72                            & 78.18                               \\
			\bottomrule
		\end{tabular}}
\end{table}
...
service is referred to as its \textbf{Fortress Index}. Table \ref{tab:fortress_index}
illustrates the stark divide between actors in the geo-arbitrage ecosystem.
...
The VPN providers' high scores reflect not enforcement of geographic
...
\begin{table}[ht]
	...
	\begin{tabularx}{\textwidth}{l X c}
		\toprule
		\textbf{Service Provider} & \textbf{Strategic Archetype} & \textbf{Fortress Index (\%)} \\
		\midrule
		ExpressVPN                & VPN Enabler                  & 55.56                        \\
		NordVPN                   & VPN Enabler                  & 50.00                        \\
		YouTube Premium           & Content Provider             & 33.68                        \\
		Microsoft                 & Software/Access              & 32.76                        \\
		Apple Music               & Content Provider             & 12.50                        \\
		Adobe                     & Software/Utility             & 5.48                         \\
		Amazon Prime              & Global Platform              & 3.88                         \\
	...
	\caption{The Fortress Index: Percentage of Enforcement Clauses
		...
		(Non-General) Sentences per Service}
...
As shown in Figure~\ref{fig:fortress_index}, VPN providers like \textbf{ExpressVPN}
and \textbf{NordVPN} had the highest density of relevant clauses, as their entire
documentation is focused on security and circumvention. Among digital services,
\textbf{YouTube} and \textbf{Microsoft} showed significantly higher
``fortress'' density than \textbf{Netflix} or \textbf{Spotify}, suggesting
a more aggressive or complex regulatory approach to user location.

\subsubsection{Strategic Framing by VPN Providers}
In sharp contrast, VPN companies adopt a ``Liberation'' and ``Privacy'' frame.
...
While many users may purchase VPNs for streaming arbitrage, providers legitimize
...
However, research has shown that not all VPN applications deliver on their
...
especially from 2022 onwards, especially for \textbf{YouTube Premium}
(see also Figure~\ref{fig:timeline_all} for the aggregate temporal evolution
...
The ``Content Providers'' (\textbf{Netflix}, \textbf{YouTube}, \textbf{Disney+})
showed a distinct move toward specialized enforcement clauses starting in 2022.
While \textbf{Spotify} remained relatively boilerplate-heavy,
\textbf{YouTube Premium} saw a massive surge in specific
``Technical Blocking'' and ``Legal Threat'' language, corresponding
to their increased efforts to combat VPN-enabled subscription hopping
in markets like Turkey and Pakistan.
...
retroactive legal threats. \gls{vpn} providers (\textbf{NordVPN},
...
(see Appendix~\ref{app:service_evolution} for VPN-specific
...
\begin{itemize}
	\item \textbf{Explicit technology prohibition:}
	      Direct references to VPNs, proxies, or location-disguising tools.
	...
	\item \textbf{Geographic conditionality:}
	      Clauses that tie service access, pricing, or content availability to the
	...
	\item \textbf{Account integrity requirements:}
	...
	\item \textbf{Compliance framing:}
	      ...
	      ($N=558$ Content Licensing clauses), reflecting the industry's
	      preferred rhetorical strategy of attributing restrictions
...
While the DSPI measured the \textit{nominal} price difference
...
for international arbitrageurs (average DSPI values of 0.45, 0.54, and
...
\begin{enumerate}
	\item \textbf{High DSPI Variance:}
	      Creates incentives for \textit{external} abuse (\gls{vpn} arbitrage).
	\item \textbf{Low Affordability:}
	      Justifies the \textit{internal} pricing strategy
	      (low nominal prices are necessary for market penetration,
	      not optional discounts).
\end{enumerate}

Thus, low nominal prices observed in the Global South are not ``bargains''
but necessary economic adjustments that inadvertently create vulnerabilities
...
\begin{table}[ht]
	...
	\begin{tabular}{l c c}
		...
		NordVPN          & 0.231                           & 5.45                                                        \\
		\midrule
		\multicolumn{3}{l}{\textbf{Pearson $r$, all $N=10$:} $-0.5493$ ($p = 0.10$, not significant at $\alpha = 0.05$)} \\
		\multicolumn{3}{l}{\textbf{Pearson $r$, excl.\ VPNs $N=8$:} $-0.0589$ ($p = 0.89$, not significant)}             \\
		\bottomrule
	\end{tabular}
	\caption{Price Discrimination Score, Enforcement Intensity, and Their Correlation.
		...
		VPN providers (low PD, high EI) and is not statistically significant at
		conventional thresholds. Excluding VPNs ($N=8$), the correlation drops to
		$r \approx -0.06$ ($p = 0.89$), indicating that among digital service providers
		alone, price discrimination and enforcement intensity are largely independent.
		Given the small sample sizes, these correlations should be interpreted as
		exploratory pattern observations rather than statistically robust evidence.}
...
Given the small sample size ($N=8$ excluding VPN providers),
these correlations should be interpreted as exploratory pattern observations rather
than statistically robust evidence; the confidence intervals at this sample size
are wide. Nevertheless, specific sector clusters emerged that showed distinct strategic behaviors.
...
\begin{itemize}
	\item \textbf{Content Providers (Netflix, Disney+, YouTube, Xbox, etc.):}
	      This group effectively formed a ``High Enforcement Cluster'' that illustrated
	      the enforcement trade-off within the streaming sector.
	      \begin{itemize}
		      \item \textbf{High Variance / High Enforcement:}
		            \textbf{YouTube} exhibits the largest combination of
		            price variance and enforcement intensity
		            (DSPI StdDev $= 0.46$, Enforcement Intensity $= 3.07\%$),
		            with a Fortress Index of 33.68\% reflecting its aggressive
		            Technical Blocking posture.
		      \item \textbf{Moderate Variance / Low Enforcement:}
		            \textbf{Disney+}, \textbf{Netflix}, and \textbf{Spotify}
		            maintain substantial price variance
		            (DSPI StdDev $0.32$--$0.49$) but rely on Content Licensing
		            ...
		            technical measures.
		      \item \textbf{Low Variance / Low Enforcement (The Xbox Case):}
		            \textbf{Xbox Game Pass} serves as a useful control case.
		            ...
		            it has relatively harmonized global pricing (DSPI StdDev $\approx 0.25$)
		            and correspondingly low enforcement intensity ($\approx 1.9\%$).
		            This suggested that when a content provider harmonized prices
		            (reducing the arbitrage incentive), the need for a ``Fortress'' strategy diminishes.
	      \end{itemize}

	\item \textbf{Utility Software (The Strategic Split):}
	      \begin{itemize}
		      \item \textbf{Adobe Creative Cloud} is a significant anomaly.
		            It rivaled Content Providers in price discrimination
		            (DSPI StdDev $\approx 0.59$) yet maintained very low ToS enforcement
		            ($\approx 0.9\%$). This confirmed the \textbf{``Utility Paradox''}:
		            downloadable software relies on cryptographic license keys
		            (``Hard'' barriers) rather than the ``Soft'' IP-blocking threats
		            required by streaming services.
	      \end{itemize}

	\item \textbf{VPN Enablers (NordVPN, ExpressVPN):} As expected, these
	      ``Adversaries'' showed minimal ``Technical Blocking'' enforcement,
	      as their business model depends on circumventing the very barriers
	      erected by the Content Providers. The legal ambiguity surrounding VPN
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Temporal Evolution of Category Incident Counts (Aggregate, Excluding VPN Providers)}
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Evolution of Strategic Frames over Time (Excluding General Terms and VPN Providers)}
...
\textbf{Adobe} showed a resurgence of ``Price Discrimination'' language in 2024,
...
Beyond the pure content and VPN providers, the ``Ecosystem'' players
...
barely changed since 2020. This stability reinforced the hypothesis that Apple relied
on hardware-locked ecosystem barriers (Apple ID region locks), or its convenience-focused ecosystem as \textbf{Business Model},
rather than dynamic \gls{tos} enforcement.
...
\begin{table}[ht]
	\centering
	\caption{Complete Fortress Index Ranking. VPN providers score highest due
...
The Fortress Index (Table \ref{tab:fortress_index_complete}) confirms the bifurcation of the market.
...
Table \ref{tab:qual_timeline_complete} reveals the temporal dynamics
underpinning these scores. The aggregate data show a clear spike
...
natural variation in document publication cadence across services. A consistent
forward-fill imputation ensures that each service's most recent \gls{tos} remains
counted in years without an update, providing an accurate picture of the active
...
The sector-level clustering patterns identified in Section~\ref{sec:correlation}
(Content Providers, Utility Software, and VPN Enablers) are equally visible
in this granular data, reinforcing the conclusion that \textbf{Business Model}
(Streaming vs.\ Download) is a stronger predictor of enforcement strategy
than \textbf{Price Opportunity} alone.
...
how do digital service providers strategically respond to VPN-enabled geo-arbitrage,
...
architecture (not the magnitude of the price gap) is the primary determinant of
...
Based on the Terms of Service analysis ($N=10$, covering 8 digital service providers
and 2 VPN enablers), we organize the findings around three high-level
\textbf{Macro-Categories} that aggregate the initial 10 coding categories
to facilitate cross-sectoral comparison:
\begin{enumerate}
	\item \textbf{Business Model Adaptation \& Pricing (BMA):}
	...
	\item \textbf{Coercive Restriction \& Legal Threat (CRL):}
	...
	\item \textbf{General Corporate Operations (GCO):}
	      Aggregates \textit{Privacy/Security} (VPN frame) and \textit{User Workaround}.
...
how these macro-strategies manifest across the analyzed companies.
...
The temporal evolution of these categories, as previously detailed in
Table \ref{tab:qual_timeline_complete}, confirms the shifting focus
toward technical countermeasures within the CRL macro-category.
...
licensing model predates digital distribution entirely, as it was designed for
...
that gives users an additional incentive to circumvent geo-blocking beyond price alone.
...
Platforms like \textbf{Apple Music} and \textbf{Amazon Prime} exemplify a ``Globalist''
...
\textcite{sinha2010drm} demonstrated that removing DRM from music files actually
\textit{increased} legitimate purchases by converting pirates into paying customers,
suggesting that overly restrictive protection can be counterproductive.
Adobe's position is best understood through the historical evolution of software protection.
In the era of physical media distribution (1990s--2000s), software piracy was rampant
because protection was limited to serial numbers and basic installation checks that
were trivially circumvented. The transition to subscription-based SaaS models
...
This reflects the ``Opportunities and Risks'' of SaaS adoption \parencite{benlian2011opportunities},
...
\begin{figure}[ht]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tikzpicture}[x=2.1cm, y=1cm]
			...
			\node[align=center, text width=2.8cm, above=1.5cm, font=\footnotesize] (pandemic) at
			(0.5,0) {\textbf{Pandemic Surge}\\(Arbitrage Wave)};
			\draw[thin] (0.5,0.2) -- (pandemic);

			\node[align=center, text width=2.8cm, above=0.5cm, font=\footnotesize] (resip) at
			(1.7,0) {\textbf{Residential IP Crackdown}\\(The ``Hard'' Filter)};
			\draw[thin] (1.7,0.2) -- (resip);

			\node[align=center, text width=2.8cm, above=1.5cm, font=\footnotesize] (youtube) at
			(2.9,0) {\textbf{YouTube Enforcement Surge}\\(Technical Blocking)};
			\draw[thin] (2.9,0.2) -- (youtube);

			\node[align=center, text width=2.8cm, above=0.5cm, font=\footnotesize] (peak) at
			(4.1,0) {\textbf{Peak Enforcement}\\(44 Tech.\ Block.\ clauses)};
			\draw[thin] (4.1,0.2) -- (peak);

			\node[align=center, text width=2.8cm, above=1.5cm, font=\footnotesize] (equilib) at
			(5.9,0) {\textbf{New Equilibrium}\\(Blocking as Standard)};
			\draw[thin] (5.9,0.2) -- (equilib);

			% Events Bottom (VPN/Adaptive)
			\node[align=center, text width=2.8cm, below=1.5cm, font=\footnotesize] (obfus) at
			(0.5,0) {\textbf{Obfuscated Servers}\\(Chameleon/XOR)};
			\draw[thin] (0.5,-0.2) -- (obfus);

			\node[align=center, text width=2.8cm, below=2.5cm, font=\footnotesize] (wireguard) at (1.7,0) {\textbf{WireGuard \& NordLynx}\\(Speed + Stealth)};
			\draw[thin] (1.7,-0.2) -- (wireguard);

			\node[align=center, text width=2.8cm, below=1.5cm, font=\footnotesize] (residential) at
			(4.1,0) {\textbf{Residential Proxies}\\(Undetectable IPs)};
			\draw[thin] (4.1,-0.2) -- (residential);

			\node[align=center, text width=2.8cm, below=2.5cm, font=\footnotesize] (dvpn) at (5.9,0) {\textbf{Decentralized VPNs}\\(Blockchain dVPNs)};
...
Servers'' (techniques analogous to those used to circumvent China's Great Firewall
...
January 2016 \parencite{netflix2016global} and the subsequent fragmentation of the
...
Our data shows a dramatic inflection point in 2022--2023, driven primarily by YouTube's
...
pseudonymous digital identities, bundling region-specific payment methods,
...
Our \gls{tos} analysis captures the \textit{stated} enforcement posture,
...
dwarfed by general terms, which constitute over 90\% of all sentences
...
Table \ref{tab:timeline_count} details the enforcement intensity per company.
Two distinct patterns emerge:
\begin{itemize}
	\item \textbf{Microsoft's Withdrawal:}
	      Microsoft shows a consistent but low-level ``background radiation''
	      of legal checks (6-12 counts), but no sudden spikes. This supports the
	      ``Enterprise Fortress'' theory—they rely on identity, not new blocking waves.
	\item \textbf{YouTube's War:} YouTube shows a massive spike in 2023 (53 counts),
	      confirming the ``Enforcement Surge'' visualized in the timeline.
	      However, this drops to 31 in 2024 and 20 in 2025, suggesting a ``cooldown''
	      period where the new blocking rules became standard practice rather than new aggressive maneuvers.
\end{itemize}
...
than pursuing a ``one-size-fits-all'' approach to geo-arbitrage. Specifically:

\begin{itemize}
	\item \textbf{Content streaming platforms}
	      ...
	      geo-blocking in the first place, effectively rendering the ``fortress'' unnecessary.
	\item \textbf{Software providers} (Adobe, Microsoft) already benefit from identity-based
	...
	\item \textbf{Ecosystem platforms} (Apple, Amazon) should recognize that their
...
privacy concerns under EU law, as the detection technologies required to enforce
...
less enforcement spending suggests a potential ``regulatory nudge'' approach:
...
content fragmentation \parencite{gomez2016geo}.
...
price data, this temporal coincidence suggests
that platforms may raise prices in ``exploited'' markets partly in response to
arbitrage losses. This creates a perverse outcome where the consumers
...
Regulation (GDPR). The irony is that privacy-enhancing technologies (\glspl{vpn})
...
shift the Pearson $r$ value, as demonstrated by the dramatic change from $r = -0.55$ (full sample)
to $r = -0.06$ (excluding VPN providers). Consequently, the findings
...
Future research could expand this dataset to include mid-tier SaaS providers,
...
(Terms of Service, annual reports, earnings calls) as proxies for corporate strategy.
...
\ref{sec:appendix_detailed_analysis} highlights this gap: the most effective enforcement
technologies are precisely those that firms have the strongest incentive to conceal.
...
The forward-fill imputation strategy used for temporal analysis also
assumes that policies remain unchanged between documented updates. This strategy
is applied consistently across all services at the individual document level:
when a specific Terms of Service document (e.g., YouTube's Paid Service Terms) is
not updated in a given year, its most recent version is carried forward until a replacement
is published. While this is a reasonable assumption---Terms of Service typically remain
legally active until explicitly revised---it may overcount the persistence of
specific clauses if internal policies change without formal document updates.
The forward-fill added 700 observations (2.8\% of the temporal dataset),
primarily affecting Spotify, Adobe, Amazon Prime, and individual YouTube ToS
document types with multi-year gaps between updates.
...
The comparison between the Gemini 3 Flash model and a traditional BERT-based
Zero-Shot classifier (Section~\ref{sec:llm_methodology}) revealed that
established NLI models are insufficient for complex legal text analysis
without extensive domain-specific fine-tuning. The BERT model achieved only
26.8\% agreement with Gemini's classifications (Cohen's Kappa = 0.032), a
level effectively equivalent to random chance. This result is consistent with
findings by \textcite{gilardi2023chatgpt}, who demonstrated that large language
models outperform traditional approaches in complex text annotation tasks.

The core distinction lies in the difference between keyword-level sensitivity
and semantic-level understanding. BERT's zero-shot approach identified surface-level
keyword matches (e.g., flagging any sentence mentioning ``account'' as an ``Account Action''),
while Gemini correctly distinguished between standard legal boilerplate that
happens to reference accounts and clauses that specifically regulate account
termination as an enforcement mechanism. For research on \gls{tos} documents
(where over 90\% of content is legal boilerplate and the analytically
relevant clauses constitute a small minority), this distinction is critical.
A model that over-classifies boilerplate as strategic content would
produce fundamentally misleading results about enforcement intensity.

This finding suggests that future research on complex legal texts should
employ advanced generative models rather than traditional NLI approaches,
...
to provide the first complete mapping of both the economic incentives driving geo-arbitrage
and the strategic responses firms deploy to counter it.

\begin{description}
	\item[\gls{rq}1 (Economic Incentive):] The \gls{dspi} reveals substantial and systematic
	      ...
	      these nominally ``cheap'' prices are often more expensive for local consumers in real terms.
	      Netflix in Argentina consumes 3.28\% of the median monthly wage, compared to 0.27\% in the United States,
	      a twelve-fold difference in economic burden. This finding fundamentally challenges the popular
	      narrative that low-income markets receive ``bargain'' prices and reframes the arbitrage
	      incentive as an exploitation of welfare-enhancing pricing structures.
	\item[\gls{rq}2 (Strategic Response):] Enforcement strategies are primarily determined by
	      \textbf{Business Model Architecture} rather than by the magnitude of price differences.
	      This is the study's most counterintuitive finding. Content streaming firms enforce strict geographic
	      ...
	      (53 Legal Threat clauses) rather than network-based blocking, reflecting the ``Enterprise Fortress'' archetype.
	...
	\item[Enforcement Evolution:] The temporal analysis reveals a clear escalation in
	      enforcement intensity, with Technical Blocking clauses increasing from 6 in 2020 to a
	      peak of 44 in 2023 before stabilizing at 24 in 2024. This surge, driven primarily by
...
era of physical media distribution and broadcast licensing, is under growing pressure
...
markets should function. The long-term trajectory, informed by the piracy parallel,
suggests that adaptive strategies (reducing the arbitrage incentive through pricing innovation)
may prove more sustainable than coercive strategies
(raising the cost of circumvention through technology), just as the music industry's
shift from \gls{drm} enforcement to convenient streaming ultimately proved more effective
at reducing piracy than ever-more-aggressive copy protection.
...
\begin{itemize}
	\item \textbf{\gls{dspi}:} We created the \gls{dspi} as a new way to measure
	...
	\item \textbf{AI Legal Analysis:} This study demonstrates that generative
	      ...
	      traditional NLI models for complex legal text classification.
	      The pipeline architecture (combining structured system prompts, batch processing,
	      and forward-fill imputation) provides a replicable template for future research
	      on corporate policy documents at scale. This contribution is particularly
	      relevant given the rapid growth of Terms of Service documents, which makes
	      traditional manual coding increasingly impractical.
\end{itemize}

\textbf{Theoretical Contributions:}
\begin{itemize}
	\item \textbf{Extension of \gls{bmi} Theory:}
	...
	\item \textbf{Strategic Archetypes:} The identification of four distinct types
	      (Content Fortress, Ecosystem Fortress, Enterprise Fortress, Utility Paradox) provides a
	      taxonomy for understanding how different business models respond to the same external threat.
	      The key finding that business model architecture is a stronger predictor of enforcement
	      strategy than price gap magnitude challenges the intuitive assumption that firms
	...
	\item \textbf{Piracy-Arbitrage Parallel:} By drawing explicit parallels between
	      ...
	      that nonetheless triggers analogous strategic responses from firms. This suggests
	...
	\item \textbf{Affordability Paradox:} The documentation of the Affordability Paradox,
...
(building on regulations such as the EU Geo-Blocking Regulation \parencite{eu2018geoblocking}),
...
\textcite{gomez2016geo} provide evidence that geo-blocking within the EU reduces
consumer welfare and digital market integration. Beyond the EU, emerging digital
...
side, emerging technologies such as decentralized \gls{vpn} networks (dVPNs) built on blockchain
...
commoditized and competition intensifies, companies may find that the costs of
...
sample beyond 11 services to include mid-tier SaaS providers, gaming platforms
...
This appendix presents evolution data for VPN providers
...
\begin{description}
	...
	\item[Anthropic Claude (Opus 4.5 and Opus 4.6)] Used as a coding assistant for the development
	      of the Python-based data collection and analysis scripts (web scraping, DSPI calculation, visualization).
