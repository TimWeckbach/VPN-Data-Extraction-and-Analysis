...
expensive for locals ($13.88\%$ of median wage across available services).
In contrast, \textbf{Turkey} ($\text{DSPI}=0.65$, \gls{ptw} $= 4.94\%$)
and \textbf{Argentina} ($\text{DSPI}=0.76$, \gls{ptw} $= 5.48\%$) show
high nominal discounts, but their \gls{ptw} values reveal that these
services still consume a significant share of local income.
