...
\newglossaryentry{forwardfill}{
	name={forward-fill imputation},
	description={A data imputation strategy applied in the longitudinal \gls{tos} analysis whereby a document's clauses are assumed to remain valid for subsequent years until a new version is detected, distinguishing between ``missing data'' and ``persistent rules''}
}
...
observations (see Chapter~\ref{chap:methodology} for the imputation rationale).
...
all 11 countries in the dataset (recall that the DSPI scale is defined in
...
\begin{table}[ht]
	...
	\begin{tabular}{l c c}
		...
		\multicolumn{3}{l}{\textbf{Pearson $r$, excl.\ VPNs $N=8$:} $+0.3467$ ($p = 0.40$, not significant)}             \\
		\bottomrule
	\end{tabular}
	\caption{Price Discrimination Score, Enforcement Intensity, and Their Correlation.
		...
		conventional thresholds. Excluding VPNs ($N=8$), the correlation reverses to a
	...
	\item \textbf{Content Providers (Netflix, Disney+, YouTube, Xbox, etc.):}
	      ...
	      \begin{itemize}
		      ...
		      \item \textbf{Moderate Variance / Low Enforcement:}
		            ...
		            technical measures.
		      \item \textbf{Moderate Variance / Low Enforcement (The Xbox Case):}
		            ...
		            (DSPI StdDev $= 0.27$), higher than Microsoft 365's own
		            ($0.21$), yet shares Microsoft's low enforcement intensity
		            (EI $= 0.87\%$).
		            This suggested that when a firm's enforcement posture is
...
providers (DSPI StdDev $= 0.21$, indicating a relatively harmonized global price for
...
(DSPI StdDev $= 0.25$, comparable to Microsoft rather than the higher-variance content
...
(see Section~\ref{sec:service_deep_dive} for the full temporal analysis).
...
to $r = +0.35$ (excluding VPN providers)---not merely a reduction in magnitude but a
...
The forward-fill imputation strategy (Chapter~\ref{chap:methodology}),
which added 700 observations (2.8\% of the temporal dataset),
assumes that policies remain unchanged between documented updates.
While reasonable---Terms of Service typically remain legally active until
explicitly revised---this may overcount the persistence of specific clauses
if internal policies change without formal document updates.
...
NLI models cannot reliably make. This is consistent with
...
\begin{description}
	\item[\gls{rq}1 (Economic Incentive):] The \gls{dspi} reveals substantial and systematic
	      ...
	      the arbitrage incentive as an exploitation of welfare-enhancing pricing structures.
...
(reducing the arbitrage incentive through pricing innovation)
