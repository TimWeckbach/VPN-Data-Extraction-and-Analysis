...
The refined analysis yields a moderate positive correlation ($R \approx 0.55$). This statistically confirms the "Fortress" hypothesis: firms with higher price variance (Netflix, Disney+) tend to employ more aggressive legal threats. However, the correlation is not perfect due to strategic outliers like \textbf{Apple Music}, which maintains high price discrimination but relies on "hard" ecosystem barriers (hardware/payment friction) rather than "soft" ToS threats (see Chapter 5 for discussion on "Ecosystem Fortresses").
