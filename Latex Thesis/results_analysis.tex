...
\subsection{The Affordability Paradox: Nominal vs. Real Cost}
While the DSPI measures the \textit{nominal} price difference (the "Tourist View" relevant to arbitrageurs), it is crucial to analyze the "Real Cost" for local residents (the "Local View"). Figure \ref{fig:affordability} maps the cost of digital services as a percentage of the \textbf{Median National Monthly Wage}. This metric serves as a digital equivalent to the "Time-to-Earn" index often used in purchasing power comparisons (e.g., the Big Mac Index's affordability variant).
...
The data reveals a paradox: while Turkey and Argentina offer the cheapest nominal prices in the world for international arbitrageurs ($<$\$4/month, DSPI $\approx 0.15$), these same services are significantly \textit{more expensive} for locals in real terms. 
For instance, a Standard Netflix subscription in Turkey consumes a significantly higher percentage of the median monthly wage ($\approx 0.6\%$) compared to the USA ($\approx 0.2\%$). 

This distinction is critical:
\begin{enumerate}
    \item \textbf{High DSPI Variance:} Creates the incentive for \textit{External} abuse (VPN Arbitrage).
...
Thus, the low nominal prices observed in the Global South are not "bargains" but necessary economic adjustments that inadvertently create the vulnerability exploited by Global North users.
